fun main() {
    val test = """[[],[10,[[6,10,0],1,[4,4,5],[4,9,4]],7,[[4,7]]],[2,[9,2],2,[]],[9,[[1,7,10,8]]]]
...
[[1,[],8,[6,10,[10,2],3]],[3],[],[[],[[4]],8]]"""
    ...
    fun parse(s: String): Node {
        val ret = ListNode()
        var cur = ret;
        val stack: MutableList<ListNode> = mutableListOf();
        stack.add(cur);
        var value = -1;
        for(c in s) {
            if (c == '[') {
                ...
                if (value >= 0) {
                    cur.children.add(ValueNode(value));
                    value = -1;
                }
                stack.removeLast();
                if (stack.size > 0) cur = stack.last();
            } else if (c.isDigit()) {
                if (value == -1) value = 0;
    ...
    fun rec(l: Node, r: Node): Int {
        if (l is ValueNode && r is ValueNode) {
            if (l.value < r.value) return -1;
            if (l.value > r.value) return 1;
            return 0;
        }
        val lList: List<Node> = if (l is ListNode) l.children else listOf(l);
        val rList: List<Node> = if (r is ListNode) r.children else listOf(r);
        for(i in 0 until lList.size) {
            if (rList.size <= i) return 1;
            val x = rec(lList[i], rList[i]);
            if (x != 0) return x;
        }
        return if (rList.size > lList.size) -1 else 0;
    }

    val sep1 = parse("[2]]");
    val sep2 = parse("[6]]");
    val nodes = mutableListOf<Node>(sep1, sep2);
    for(x in test.split("\n")) {
        if (x == "") continue;
        nodes.add(parse(x.substring(1)));
    }
    val comparator = Comparator { a: Node, b: Node -> rec(a, b) };
    val sorted = nodes.sortedWith(comparator);
    println((sorted.indexOf(sep1) + 1) * (sorted.indexOf(sep2) + 1));
}
