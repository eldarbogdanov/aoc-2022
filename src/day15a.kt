...
Sensor at x=3891093, y=2110588: closest beacon is at x=3748004, y=2000000"""
    ...
    for(x in -10000000..10000000) {
        ...
        var definite = false;
        for(pair in data) {
            if (pair.second.first == x && pair.second.second == testY) {
                definite = true;
                continue;
            }
            val d = manhattan(pair.first, pair.second);
            if (abs(pair.first.second - testY) > d) continue;
            val free = d - abs(pair.first.second - testY);
            if (x in pair.first.first - free..pair.first.first + free) {
                can = false;
//                println(x);
//                println(pair);
            };
